...
class AttachmentsHelper {
  ...
  private static function parse_url(&$raw_url, $relative_url) {
    ...
    if (preg_match('^(?P<protocol>\b[A-Z]+\b://)?'
                    ...
                    . ':?(?P<port>[0-9]*)'
                    . '(?P<path>/[-A-Z0-9+&@#/%=~_|!:,.;]*)'
                    . '?(?P<parameters>\?[-A-Z0-9+&@#/%=~_|!:,.;]*)?^i', $url, $match)) {
  ...
  /**
   * Trigger a new events for attachments 
   * https://docs.joomla.org/Supporting_plugins_in_your_component
   * @param  $attachment object that the event is related
   * @param string $event type of event
   *  
   * @return void
   * 
   */
  public static function newEvent($attachment, $event) {
    ...
    switch ($event) {
      case 'onContentAfterSave':
        $result = $dispatcher->trigger('onContentAfterSave', array($context, &$table, $isNew));

        break;
      case 'onContentBeforeSave':
        $result = $dispatcher->trigger('onContentBeforeSave', array($context, &$table, $isNew));

        break;
      case 'onContentAfterDelete':
        $result = $dispatcher->trigger('onContentAfterDelete', array($context, &$table, $isNew));
  ...
  /**
   * Trigger a new event for changing state on several attachment in the admin menu 
   * https://docs.joomla.org/Supporting_plugins_in_your_component
   * @param  $cid  array of ids of attachments selected
   * @param string $value value of the status changing to
   *  
   * @return void 
   * 
   */
  public static function eventChangeState(&$cid, $value) {
    $dispatcher = JEventDispatcher::getInstance();
    $context = 'com_attachments';
    $result = $dispatcher->trigger('onContentChangeState', array($context, $cid, $value));
  }
